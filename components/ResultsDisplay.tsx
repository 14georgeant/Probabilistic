...
const ResultsDisplay: React.FC<ResultsDisplayProps> = ({ result, insights, isLoading, variables, mode = 'general', onExportCpp }) => {
    ...
    const getColors = () => {
        ...
        if (mode === 'programmer') return { main: 'lime', hex: '#84CC16' };
        if (mode === 'mental') return { main: 'amber', hex: '#F59E0B' };
    ...
    const handleExportNotebookLM = () => {
        ...
        if (mode === 'programmer') title = "ENGINEERING ARCHITECTURE REPORT";
        if (mode === 'mental') title = "WELLNESS & RESILIENCE REPORT";
...
OPTIMAL ${mode === 'financial' ? 'PORTFOLIO/PATH' : mode === 'health' ? 'TRAINING PROTOCOL' : mode === 'medical' ? 'CLINICAL PATH' : mode === 'programmer' ? 'TECH STACK' : mode === 'mental' ? 'WELLNESS PLAN' : 'STRATEGY PATH'}
---------------------
...
`;
    ...
    const handleCopySocial = () => {
        ...
        if (mode === 'financial') {
            ...
            headerIcon = '👨‍💻';
            footerTags = '#DevOps #Engineering #SystemDesign';
            contextText = 'Tech Architecture:';
        } else if (mode === 'mental') {
            headerIcon = '🧘';
            footerTags = '#MentalWellness #Mindfulness #Resilience';
            contextText = 'Wellness Plan:';
        }
...
📊 ${mode === 'health' ? 'Performance Probability' : mode === 'medical' ? 'Prognosis Confidence' : mode === 'programmer' ? 'Ship Confidence' : mode === 'mental' ? 'Balance Probability' : 'Confidence Score'}: ${(result.highestProbability * 100).toFixed(2)}%
            ...
            <div className="flex flex-col items-center justify-center h-full text-center">
                ...
                <p className="text-sm text-gray-400">
                    {mode === 'financial' ? 'Consulting financial algorithms...' 
                     ...
                     : mode === 'programmer' ? 'Reviewing architecture and code patterns...'
                     : mode === 'mental' ? 'Reflecting on wellness factors...'
        ...
        <div className="space-y-8 animate-fade-in">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-gray-700 pb-4 gap-4">
                <h3 className="text-xl font-bold text-white">
                    {mode === 'financial' ? 'Projected Confidence:' : mode === 'health' ? 'Performance Probability:' : mode === 'medical' ? 'Prognosis Confidence:' : mode === 'programmer' ? 'Ship Confidence:' : mode === 'mental' ? 'Balance Probability:' : 'Optimal Path:'} <span className={`text-${theme.main}-400`}>{(result.highestProbability * 100).toFixed(2)}%</span>
                </h3>
            ...
            <div className="bg-gray-800/80 rounded-lg border border-gray-700 p-6">
                <h4 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
                    {mode === 'financial' ? 'Risk Exposure Analysis' : mode === 'health' ? 'Protocol Bottlenecks' : mode === 'medical' ? 'Clinical Contraindications' : mode === 'programmer' ? 'Tech Debt Risks' : mode === 'mental' ? 'Resilience Factors' : 'Leverage Analysis'}
                </h4>
                <p className="text-sm text-gray-400 mb-4">
                    {mode === 'financial' 
                        ...
                        : mode === 'programmer'
                        ? 'Identifying the riskiest parts of the stack. Refactoring these items offers the highest ROI.'
                        : mode === 'mental'
                        ? 'Identifying areas where small changes can bring the most peace of mind.'
            ...
            {insights && (
                <div className="mt-8">
                    <h3 className="text-xl font-bold text-white mb-3 flex items-center border-t border-gray-700 pt-6">
                        ...
                        Gemini {mode === 'financial' ? 'Advisory' : mode === 'health' ? 'Performance Coach' : mode === 'medical' ? 'Clinical Assessment' : mode === 'programmer' ? 'Code Architecture Review' : mode === 'mental' ? 'Wellness Coach' : 'Strategic Insights'}
                    </h3>
