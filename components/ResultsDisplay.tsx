...
const ResultsDisplay: React.FC<ResultsDisplayProps> = ({ result, insights, isLoading, variables, mode = 'general' }) => {
    ...
    const themeColor = mode === 'financial' ? 'emerald' : 'cyan';
    const barColor = mode === 'financial' ? '#10B981' : '#22D3EE'; // Emerald-500 vs Cyan-400
    ...
    const handleExportNotebookLM = () => {
        ...
        const title = mode === 'financial' ? "FINANCIAL ADVISORY REPORT" : "STRATEGIC OUTCOME REPORT";
...
OPTIMAL ${mode === 'financial' ? 'PORTFOLIO/PATH' : 'STRATEGY PATH'}
---------------------
Achieves ${(result.highestProbability * 100).toFixed(2)}% Probability of ${result.outcomeName}.

${result.bestCombination.map(c => `• ${c.variableName}: Choose "${c.stateName}" (Base Prob: ${c.baseProbability}%)`).join('\n')}
...
`;
    ...
    const handleCopySocial = () => {
        ...
        const headerIcon = mode === 'financial' ? '📈' : '🚀';
        const footerTags = mode === 'financial' ? '#Finance #Investing #WealthManagement' : '#Strategy #AI #Optimization';
        
        const socialText = `${headerIcon} ${mode === 'financial' ? 'Financial Outlook:' : 'Optimized Strategy:'} "${result.outcomeName}"

✨ Winning Path:
${result.bestCombination.map(c => `👉 ${c.variableName}: ${c.stateName}`).join('\n')}

📊 ${mode === 'financial' ? 'Projected Confidence' : 'Success Probability'}: ${(result.highestProbability * 100).toFixed(2)}%
...
Analyzed via Probabilistic Outcome Analyzer ${footerTags}`;
    ...
    const renderMarkdown = (text: string) => {
        const headingColor = mode === 'financial' ? 'text-emerald-400' : 'text-cyan-400';
        const subHeadingColor = mode === 'financial' ? 'text-emerald-300' : 'text-cyan-300';
    ...
    if (isLoading) {
        return (
            <div className="flex flex-col items-center justify-center h-full text-center">
                <div className={`animate-spin rounded-full h-16 w-16 border-t-2 border-b-2 ${mode === 'financial' ? 'border-emerald-500' : 'border-cyan-500'} mb-4`}></div>
                <p className="text-lg font-semibold text-gray-300">Running probabilistic model...</p>
                <p className="text-sm text-gray-400">{mode === 'financial' ? 'Consulting financial algorithms...' : 'Querying Gemini for strategic insights...'}</p>
            </div>
        );
    }
        ...
        <div className="space-y-8 animate-fade-in">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-gray-700 pb-4 gap-4">
                <h3 className="text-xl font-bold text-white">
                    {mode === 'financial' ? 'Projected Confidence:' : 'Optimal Path:'} <span className={mode === 'financial' ? 'text-emerald-400' : 'text-cyan-400'}>{(result.highestProbability * 100).toFixed(2)}%</span>
                </h3>
                ...
                <div className="flex flex-wrap gap-2">
                    ...
                    <button
                        ...
                        className={`flex items-center gap-2 text-xs font-medium uppercase tracking-wider py-2 px-3 rounded-md transition-all shadow-sm ${mode === 'financial' ? 'bg-gray-700 hover:bg-gray-600 text-emerald-400 border border-emerald-900' : 'bg-gray-700 hover:bg-gray-600 text-cyan-400 border border-gray-600'}`}
                    >
                        JSON
                    </button>
                </div>
                        ...
                        {result.bestCombination.map((item, idx) => (
                            <div key={idx} className="bg-gray-800/50 p-3 rounded-md">
                                <div className="flex justify-between text-sm mb-1">
                                    ...
                                    <span className={`${mode === 'financial' ? 'text-emerald-300' : 'text-cyan-300'} font-mono`}>{simulatedValues[item.variableName] ?? item.baseProbability}%</span>
                                </div>
                                <input 
                                    type="range" 
                                    min="0" 
                                    ...
                                    className={`w-full h-2 bg-gray-600 rounded-lg appearance-none cursor-pointer ${mode === 'financial' ? 'accent-emerald-500' : 'accent-cyan-500'}`}
                                />
                ...
                <div className="flex flex-col">
                    ...
                    <div className="flex-grow bg-gray-700/30 p-4 rounded-lg border border-gray-600/50 flex flex-col justify-center">
                                ...
                                <BarChart data={chartData} layout="vertical" margin={{ top: 5, right: 30, left: 40, bottom: 5 }}>
                                    ...
                                    <Bar dataKey="value" radius={[0, 4, 4, 0]} barSize={30}>
                                        {chartData.map((entry, index) => (
                                            <Cell key={`cell-${index}`} fill={entry.type === 'Current' ? '#4B5563' : (probDelta >= 0 ? barColor : '#EF4444')} />
                                        ))}
                                    </Bar>
                                </BarChart>
                            </ResponsiveContainer>
                        ...
                        <div className="text-center">
                            ...
                            <div className={`text-4xl font-bold ${probDelta > 0 ? (mode === 'financial' ? 'text-emerald-400' : 'text-cyan-400') : probDelta < 0 ? 'text-red-400' : 'text-white'}`}>
            ...
            <div className="bg-gray-800/80 rounded-lg border border-gray-700 p-6">
                <h4 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
                    {mode === 'financial' ? 'Risk Exposure Analysis' : 'Leverage Analysis (Room for Improvement)'}
                </h4>
                <p className="text-sm text-gray-400 mb-4">
                    {mode === 'financial' 
                        ? 'Identifying high-risk factors. Variables at the top have the lowest confidence and may require hedging or diversification.'
                        : 'Identifying the "Weakest Links". Variables at the top have the lowest current probability and offer the highest ROI for improvement efforts.'}
                </p>
            ...
            {insights && (
                <div className="mt-8">
                    <h3 className="text-xl font-bold text-white mb-3 flex items-center border-t border-gray-700 pt-6">
                         <svg className={`w-6 h-6 mr-2 ${mode === 'financial' ? 'text-emerald-400' : 'text-cyan-400'}`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                           <path strokeLinecap="round" strokeLinejoin="round" d="M9.813 15.904 9 18.75l-.813-2.846a4.5 4.5 0 0 0-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 0 0 3.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 0 0 3.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 0 0-3.09 3.09ZM18.259 8.715 18 9.75l-.259-1.035a3.375 3.375 0 0 0-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 0 0 2.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 0 0 2.456 2.456L21.75 6l-1.035.259a3.375 3.375 0 0 0-2.456 2.456Z" />
                        </svg>
                        Gemini {mode === 'financial' ? 'Advisory' : 'Strategic Insights'}
                    </h3>
